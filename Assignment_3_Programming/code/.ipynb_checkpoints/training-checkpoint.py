...
# Training model
def train_model(model, optim, loss_fn, train_iter, mode, epsilon=0.01):
    total_epoch_loss = 0
    total_epoch_acc = 0
    steps = 0
    model.train()
...
# Test model
def eval_model(model, loss_fn, test_iter, mode):
    total_epoch_loss = 0
    total_epoch_acc = 0
    model.eval()
    r = 0
...
def main(trainPlain = True, trainAdv = True, trainProx = True):
    ...
    if trainPlain:
        ...
        for epoch in range(basic_epoch):
                ...
                train_loss, train_acc = train_model(model, optim, loss_fn, train_iter, mode = 'plain')
                val_loss, val_acc = eval_model(model, loss_fn, test_iter, mode ='plain')
                if epoch%50 == 0:
    ...
    if trainAdv:
        ...
        for ind, eps in enumerate(epsilons):
            ...
            for epoch in range(Adv_epochs[ind]):
                ...
                train_loss, train_acc = train_model(Adv_model, optim, loss_fn, train_iter, mode = 'AdvLSTM', epsilon=eps)
                val_loss, val_acc = eval_model(Adv_model, loss_fn, test_iter, mode = 'AdvLSTM')
    ...
    if trainProx:
        ...
        for epoch in range(Prox_epoch):
            ...
            train_loss, train_acc = train_model(Prox_model, optim, loss_fn, train_iter, mode = 'ProxLSTM')
        ...
        for ep in eps:
            ...
            val_loss, val_acc = eval_model(Prox_model, loss_fn, test_iter, mode ='ProxLSTM')
...
if __name__ == '__main__':
    main(trainPlain = True, trainAdv= True, trainProx = False)
