...
class CRF_Layer(nn.Module):
    ...
    def logPYX(self, m, data, label, w, T, alpha, dots):
    ...
    def computeDP(self, m, data, w, T, dots):
    ...
    def computeMarginal(self, m, data, w, T, alpha, beta, dots):
    ...
    def computeGradientWy(self,m, data, label, p1):
    ...
    def computeGradientTij(self,m, data, label, p2):
    ...
    def forward(self, dataset):
        """ DESCRIPTION: (old backward). THis function will compute all the marginals and gradients for each
                         batch. The grads are stored in the class.
        """
        ...
        label_s = [dataset[i][0] for i in range(len(dataset))]
        data_s = [dataset[i][1] for i in range(len(dataset))]

        label_s = torch.stack(label_s)
        data_s = torch.stack(data_s)
        ...
        dots_ = (data_s @ self.w.T).transpose(1,2)
        ...
        for i in range(len(dataset)):
            ...
            alpha, beta = self.computeDP(m, data, w, T, dots)
            p1, p2 = self.computeMarginal(m, data, w, T, alpha, beta, dots)

            dw = self.computeGradientWy(m, data, label, p1)
            dT = self.computeGradientTij(m, data, label, p2)

            meanLogPYX += self.logPYX(m, data, label, w, T, alpha, dots)
            meandw += dw
            meandT += dT
